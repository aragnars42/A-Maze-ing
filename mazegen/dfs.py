...
class MazeGenerator:
    ...
    def __init__(self, width: int, height: int, seed: Any | None) -> None:
        ...
        Args:
            ...
            seed (Any|None): Define predictable randomness
            (same maze if the seed is repeated).
        """
    ...
    def make_imperfect(self, openings: int = 200) -> None:
        """
        ...
        for _ in range(openings):
            ...
            for d in directions:
                ...
                if (
                    self.maz.has_wall(x, y, d["direc"]) and
                    self.maz.cells[y][x] not in (1, 2, 4, 8) and
                    self.maz.cells[ny][nx] not in (1, 2, 4, 8)
                ):
                    self.maz.open_wall(x, y, d["direc"])
                    self.maz.open_wall(nx, ny, d["opposite"])
                    break
