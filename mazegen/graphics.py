...
class Graphics:
    def __init__(self, maze, entry: Tuple[int, int], exit_pos: Tuple[int, int], path: str, regen_callback=None):
        self.mlx: Mlx = None
    ...
    def draw_entry_exit(self) -> None:
        ...
        for screen_x in range(entry_x, entry_x + self.cell_size):
            for screen_y in range(entry_y, entry_y + self.cell_size):
                self.mlx.mlx_pixel_put(self.mlx_ptr, self.window, screen_x, screen_y, COLOR_PALETTES[self.color_idx][1])
        ...
        for screen_x in range(exit_x, exit_x + self.cell_size):
            for screen_y in range(exit_y, exit_y + self.cell_size):
                self.mlx.mlx_pixel_put(self.mlx_ptr, self.window, screen_x, screen_y, COLOR_PALETTES[self.color_idx][2])
            ...
            if 0 <= current_x < self.maze.width and 0 <= current_y < self.maze.height:
                ...
                for screen_x in range(pixel_x, pixel_x + self.cell_size):
                    for screen_y in range(pixel_y, pixel_y + self.cell_size):
                        self.mlx.mlx_pixel_put(self.mlx_ptr, self.window, screen_x, screen_y, COLOR_PALETTES[self.color_idx][3])
        ...
        for y in range(0, self.maze.height):
            for x in range(0, self.maze.width):
                ...
                if cell == 15:
                    for screen_x in range(pixel_x, pixel_x + self.cell_size):
                        for screen_y in range(pixel_y, pixel_y + self.cell_size):
                            self.mlx.mlx_pixel_put(self.mlx_ptr, self.window, screen_x, screen_y, COLOR_PALETTES[self.color_idx][0])
                    continue

                # Otherwise draw individual walls as before
                if self.maze.has_wall(x, y, 0):
                    for screen_x in range(pixel_x, pixel_x + self.cell_size):
                        self.mlx.mlx_pixel_put(self.mlx_ptr, self.window, screen_x, pixel_y, COLOR_PALETTES[self.color_idx][0])
                if self.maze.has_wall(x, y, 1):
                    for screen_y in range(pixel_y, pixel_y + self.cell_size):
                        self.mlx.mlx_pixel_put(self.mlx_ptr, self.window, pixel_x + self.cell_size, screen_y, COLOR_PALETTES[self.color_idx][0])
                if self.maze.has_wall(x, y, 2):
                    for screen_x in range(pixel_x, pixel_x + self.cell_size):
                        self.mlx.mlx_pixel_put(self.mlx_ptr, self.window, screen_x, pixel_y + self.cell_size, COLOR_PALETTES[self.color_idx][0])
                if self.maze.has_wall(x, y, 3):
                    for screen_y in range(pixel_y, pixel_y + self.cell_size):
                        self.mlx.mlx_pixel_put(self.mlx_ptr, self.window, pixel_x, screen_y, COLOR_PALETTES[self.color_idx][0])

    def draw_hint(self) -> None:
        hint_y = self.maze.height * self.cell_size + 5
        self.mlx.mlx_string_put(self.mlx_ptr, self.window, 5, hint_y, 0xFFFFFFFF,
    ...
    def key_code(self, keycode: int, param) -> None:
        match keycode:
            ...
            case KEY_CODES.KEY_3:
                self.color_idx = (self.color_idx + 1) % len(COLOR_PALETTES)
                self.mlx.mlx_clear_window(self.mlx_ptr, self.window)
                self.draw_maze()
                if self.path_visible:
                    self.draw_path()
