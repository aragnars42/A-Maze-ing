...
class Graphics:
	def __init__(self, maze, entry: Tuple[int, int], exit_pos: Tuple[int, int], path: str):
		...
		try:
			...
			self.window_height = self.maze.height * self.cell_size + 1
	...
	def render(self) -> None:
		...
		self.mlx.mlx_key_hook(self.window, self.mlx.mlx_destroy_window, 65307)
		self.mlx.mlx_key_hook(self.window, self.mlx.mlx_destroy_window, 17)
		self.mlx.mlx_loop(self.mlx_ptr)
		self.mlx.mlx_destroy_window(self.mlx_ptr, self.window)
		self.mlx.mlx_destroy_image(self.mlx_ptr, self.window)